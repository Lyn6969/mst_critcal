function results = run_delta_c_m_vs_1_scan_event_with_N(N_input, varargin)
%RUN_DELTA_C_M_VS_1_SCAN_EVENT_WITH_N
% 在 run_delta_c_m_vs_1_scan_parallel_with_N 基础上，仅修改分支比计算为“事件级”：
%   - 每一次激活事件都计入分子/分母，同一节点多次激活也会重复计数。
%   - 其他参数、流程、输出字段保持不变，便于与节点级（unique）和原版对比。
%
% 外源触发事件是否记入：
%   - 本实现将外源触发后首次激活的“种子激活”也计为事件（符合 Hawkes/分支过程习惯）。
%     如需排除，可在 compute_branching_ratio_with_cascade 内筛掉种子事件。
%
% 使用方式同原脚本：run_delta_c_m_vs_1_scan_event_with_N(N [, eta])

%% 1. 实验设定
fprintf('=================================================\n');
fprintf('   Delta_c (1 vs m) 参数扫描 (事件级分支比) (N=%d)\n', N_input);
fprintf('=================================================\n\n');
...
if ~isempty(eta_value)
    angle_noise_intensity = (eta_value.^2) / 2;
...
%% 6. 保存
results = struct();
results.description = sprintf('Delta_c parameter scan (1 vs m, event-level b) with N=%d', N_input);
...
function [ratio, cascade_size] = compute_branching_ratio_with_cascade(sim, pulse_count)
    % 事件级分支比：每一次激活事件都计入（同一节点多次激活会重复计数）。
    % 外源触发后的首次激活也算事件；如需排除，可在 valid_new 时过滤 seeds。
    ...
    if parent_event_count == 0
        ...
        ratio = children_event_sum / parent_event_count;
    end
...
function render_quicklook_figure_with_N(thresholds, pulse_counts, c_mean, c_sem, ...
    ...
    sgtitle(sprintf('Delta_c (1 vs m) 参数扫描 (事件级分支比) (N=%d, %d次重复, %d workers)', ...
...
function output_dir = ensure_data_directory_with_N(timestamp, N_input, eta_tag)
    % 事件级版本输出目录增加 event 标识
    base_dir = fullfile(pwd, 'data', 'experiments', 'delta_c_m_vs_1_scan');
    if ~isfolder(base_dir)
        mkdir(base_dir);
    end
    if nargin < 3 || isempty(eta_tag)
        folder_name = sprintf('N%d_event_%s', N_input, timestamp);
    else
        folder_name = sprintf('N%d_event_%s_%s', N_input, eta_tag, timestamp);
    end
