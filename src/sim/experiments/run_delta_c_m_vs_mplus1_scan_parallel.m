...
%% 1. 基本设定
fprintf('=================================================\n');
fprintf('   Delta_c (m vs m+1) 参数扫描实验 (并行化版本)\n');
fprintf('=================================================\n\n');
...
fprintf('参数点数: %d, 重复次数: %d, 总实验数: %d\n', ...
    num_params, num_runs, total_tasks);
fprintf('覆盖初发个体数: %s\n', mat2str(pulse_counts));
...
delta_c = zeros(num_params, numel(m_values));
delta_sem = zeros(num_params, numel(m_values));
for idx = 1:numel(m_values)
    m = m_values(idx);
    delta_c(:, idx) = c_mean(:, m + 1) - c_mean(:, m);
    delta_sem(:, idx) = sqrt(c_sem(:, m + 1).^2 + c_sem(:, m).^2);
end

%% 6. 保存
results = struct();
results.description = 'Delta_c parameter scan (m vs m+1)';
...
optimal_m = m_values(max_m_idx);
fprintf('Δc 峰值: %.4f (cj = %.2f, m = %d → %d)\n', ...
    max_delta, optimal_cj, optimal_m, optimal_m + 1);
...
function render_quicklook_figure(thresholds, m_values, c_mean, c_sem, ...
    delta_c, delta_sem, error_count, num_runs, num_workers, output_prefix)

    fig = figure('Name', 'Delta_c (m vs m+1) 扫描结果', ...
        'Position', [100, 100, 1350, 450]);

    subplot(1, 3, 1);
    hold on;
    colors = lines(size(c_mean, 2));
    for idx = 1:size(c_mean, 2)
        if idx == 1 || idx > 6 && mod(idx,2)==1
            continue;
        end
    ...
    for idx = 1:numel(m_values)
        errorbar(thresholds, delta_c(:, idx), delta_sem(:, idx), '-', ...
            'LineWidth', 1.2, ...
            'DisplayName', sprintf('\\Delta c_{%d\\rightarrow%d}', m_values(idx), m_values(idx)+1));
    end
    xlabel('c_j threshold');
    ylabel('\Delta c = c_{m+1} - c_m');
    title('集群敏感性 (m vs m+1)');
    legend('Location', 'northeastoutside');
    grid on;
    hold off;
...
function output_dir = ensure_data_directory(timestamp)
    base_dir = fullfile(pwd, 'data', 'experiments', 'delta_c_m_vs_mplus1_scan');
