...
%RUN_DELTA_C_M_VS_1_SCAN_PARALLEL 并行扫描运动显著性阈值对多初发敏感性的影响。
%   在基准 c_1 的基础上，依次比较 c_m (m=2…10) 与 c_1 的差值，得到
%   Δc_{m,1} = c_m - c_1，多角度复现 Fig. S11 中的敏感性分析。
...
function [ratio, cascade_size] = compute_branching_ratio_with_cascade(sim, pulse_count)
    % 复制自 compute_branching_ratio，并返回最终级联规模
