function results = run_delta_c_m_vs_1_scan_unique_with_N(N_input, varargin)
%RUN_DELTA_C_M_VS_1_SCAN_UNIQUE_WITH_N
% 在 run_delta_c_m_vs_1_scan_parallel_with_N 的基础上，仅修改分支比计算：
%   - 只统计“首次激活”的子代，后续重复激活不再累加，避免高估分支比。
%   - 其他参数、流程、输出字段保持不变，方便与原版直接对比。
%
% 使用方式、输入输出与原文件一致；若需比较两种定义的分支比，可分别运行原脚本与本脚本。
%
% 输入参数:
...
%
% 分支比定义差异（重点）：
%   - 原版：同一节点若多次激活，每次都会为其父节点贡献 1 个子代计数，可能抬高 b。
%   - 本版：同一节点只在“首次激活”时贡献子代，重复激活被忽略；分母仍为所有被激活过的唯一节点。

%% 1. 实验设定
fprintf('=================================================\n');
fprintf('   Delta_c (1 vs m) 参数扫描 (唯一首次激活计算) (N=%d)\n', N_input);
fprintf('=================================================\n\n');
...
%% 6. 保存
results = struct();
results.description = sprintf('Delta_c parameter scan (1 vs m, unique-activation b) with N=%d', N_input);
...
function [ratio, cascade_size] = compute_branching_ratio_with_cascade(sim, pulse_count)
    % 与原版最大的区别：只统计“首次激活”的子代，重复激活不计入分子。
    ...
    parent_flags = false(sim.N, 1);    % 记录曾被激活过（分母用）
    children_count = zeros(sim.N, 1);  % 每个父节点的唯一子代计数
    ever_child = false(sim.N, 1);      % 每个节点是否已作为“子”被计过
    ...
    for step = 1:max_remaining_steps
        ...
        if counting_enabled && sim.current_step <= tracking_deadline && any(newly_active)
            % 只统计首次激活的节点，重复激活被过滤
            valid_new = newly_active & ~ever_child;
            ever_child(valid_new) = true;

            activated_indices = find(valid_new);
...
%RENDER_QUICKLOOK_FIGURE_WITH_N 生成包含N值信息的Delta_c扫描预览图
    ...
    sgtitle(sprintf('Delta_c (1 vs m) 参数扫描 (唯一首次激活) (N=%d, %d次重复, %d workers)', ...
